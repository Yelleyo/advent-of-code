...
def main():
    ...
    root = int(check_size(block_dict, '/'))
    print(f'This is the size of root: {root}')
    space_to_clean = abs(40000000 - root)
    print(f'We need to clean this amount of space: {space_to_clean}')

    folders_potential = []

    for key in block_dict:
        if int(check_size(block_dict, key)) >= space_to_clean:
            folders_potential.append(int(check_size(block_dict, key)))
    
    folders_potential.sort()
    print(f'the smallest folder would be this size: {folders_potential[0]}')
